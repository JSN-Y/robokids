...
| `(root)`      | The frontend game client interface.                                      |
...
1. **Clone the repository**
   ...
   ```bash
   git clone https://github.com/JSN-Y/robokids.git
   cd robokids
   ```

2. **Install all workspace dependencies**

   This command reads `pnpm-workspace.yaml` and installs dependencies for the API, shared libraries, and the client simultaneously.
...
- **Backend (`/api`)** — Create a `.env` file inside the `api` directory and add your database connection string, JWT secrets, and port configuration.
- **Frontend (root)** — Create a `.env` file in the root (or client directory) containing your local API endpoint URL, e.g.:
...
1. **Start the API server** *(always start this first, so the game client can fetch user data on load)*:
...
2. **Start the game client** — in a new terminal tab/window, from the project root:
...
- **Frontend / Game Client** — Automated deployments via [Vercel](https://vercel.com) (`vercel.json`).
- **Backend / API** — Deployed via [Render](https://render.com) (`render.yaml`).

## 🧑‍💻 Author

**Yassine Abid**
GitHub: [@JSN-Y](https://github.com/JSN-Y)
